...
var embed_token_invalid = new Discord.MessageEmbed()
    ...
    .addField(`Using the command :`, `!pub bot token`)
    .setColor(config.embedColor)

var embed_pub_s1_stopped = new Discord.MessageEmbed()
    .setTitle(`** ${config.m_pub_s1_stopped} **`)
    .setFooter("You can start advertising again now")
    .setColor(config.embedColor)
                        ...
                        .then(async collected => {
                            ...
                            if(pub2.startsWith("{") && pub2.endsWith("}")){
                                ...
                                .then(async collected => {
                                    ...
                                    if (reaction.emoji.name === '✅') {
                                        let mbr = await newClient[message.channel.id].users.cache.filter(member => !member.bot).size
                                       ...
                                       let msg = await message.channel.send(new Discord.MessageEmbed()
                                        ...
                                        let memberarray = newClient[message.channel.id].users.cache.array();
